# Griffin31 ToolKit

Free tools by Griffin31 to help M365 admins stay ahead of Microsoft changes and strengthen their security posture.

These tools complement the [Griffin31](https://griffin31.com) SaaS platform for M365 security posture management, alerts, audit, and compliance.

## Tools

| Tool | Description |
|------|-------------|
| [CA-Update-AffectedApps](CA-Update-AffectedApps/) | Identifies tenant apps affected by Microsoft's upcoming Conditional Access enforcement change (May 2026). |
| [EXO-AppPermissions-Manager](EXO-AppPermissions-Manager/) | Automates Exchange Online app-to-mailbox permission scoping — roles, management scopes, and verification. |
| [SPF-Lookup-Validator](SPF-Lookup-Validator/) | Recursively analyzes SPF records, counts DNS lookups, and validates RFC 7208 compliance. |
| [Entra-StaleDevices-Cleanup](Entra-StaleDevices-Cleanup/) | Audit, disable, or delete stale devices in Entra ID — filter by OS, inactivity threshold, and action mode. |
